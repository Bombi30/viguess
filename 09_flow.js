  ...
  async function fetchImagesForLocation(loc) {
    ...
    const fetchPromise = (async () => {
      async function queryBBox(lat, lng, delta) {
        ...
        const timeoutId = setTimeout(() => controller.abort(), 3500); // Giảm timeout xuống 3.5s
        try {
      ...
      // 2. If empty, query 8 small offsets in parallel to save time
      if (images.length === 0) {
        const o1 = 0.0008;
        const o2 = 0.0016;
        const results = await Promise.all([
          queryBBox(loc.lat + o1, loc.lng, 0.0005),
          queryBBox(loc.lat - o1, loc.lng, 0.0005),
          queryBBox(loc.lat, loc.lng + o1, 0.0005),
          queryBBox(loc.lat, loc.lng - o1, 0.0005),
          queryBBox(loc.lat + o2, loc.lng, 0.0005),
          queryBBox(loc.lat - o2, loc.lng, 0.0005),
          queryBBox(loc.lat, loc.lng + o2, 0.0005),
          queryBBox(loc.lat, loc.lng - o2, 0.0005)
        ]);
        images = results.flat();
      }
  ...
  function setupConnection() {
    ...
    peerConnection.on('data', data => {
        ...
        if (data.type === 'START_GAME') {
            ...
            alert("Đối thủ đã quay về sảnh để thay đổi cài đặt!");
            resetGame();
            goToLobby();
        } else if (data.type === 'OPPONENT_QUIT') {
            alert("Đối thủ đã thoát khỏi phòng!");
            cleanupPeer();
            resetGame();
            showScreen('start');
            ...
            if (data.round === currentRound) {
                ...
                // Host tự quyết định việc đổi ảnh dựa trên kết quả load của chính mình.
                // Không đổi ảnh chỉ vì đối thủ load lỗi (tránh loop đổi ảnh liên tục).
            }
        }
    });
  ...
  function startMultiplayerRound() {
    ...
    if (btnSkillDizzy) {
        btnSkillDizzy.style.display = 'inline-block';
        btnSkillDizzy.disabled = false;
    }
    const btnSkillTunnel = $('btn-skill-tunnel');
    if (btnSkillTunnel) {
        btnSkillTunnel.style.display = 'inline-block';
        btnSkillTunnel.disabled = false;
    }
  ...
  function loadRound() {
    ...
    if (btnSkill) {
        if (isEndless) {
            btnSkill.style.display = 'inline-block';
            btnSkill.disabled = false;
            if (btnSkillDizzy) { btnSkillDizzy.style.display = 'inline-block'; btnSkillDizzy.disabled = false; }
            if (btnSkillTunnel) { btnSkillTunnel.style.display = 'inline-block'; btnSkillTunnel.disabled = false; }
        } else if (!isMultiplayer) {
            btnSkill.style.display = 'none';
            if (btnSkillDizzy) btnSkillDizzy.style.display = 'none';
            if (btnSkillTunnel) btnSkillTunnel.style.display = 'none';
        }
    }
